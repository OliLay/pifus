...
file_prefix = "dummy_sync_competing"

def measure():
    """Stack and three competing dummies with different prios."""
    framework.start_stack(affinity="0-4")

    pifus_dummy_path = framework.get_binary_path(
        "api/benchmarks/dummy/pifus_dummy_sync")
    framework.start_process(
        pifus_dummy_path, args=f"-l HIGH -o {file_prefix}_HIGH", affinity="5")
    framework.start_process(
        pifus_dummy_path, args=f"-l MEDIUM -o {file_prefix}_MEDIUM", affinity="6")
    framework.start_process(
        pifus_dummy_path, args=f"-l LOW -o {file_prefix}_LOW", affinity="7")
