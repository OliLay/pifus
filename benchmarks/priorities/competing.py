...
def measure():
    ...
    framework.start_process(
        lwip_reader_path, args="192.168.1.202", tapif=2, affinity="3"
    )
    framework.start_process(
        lwip_reader_path, args="192.168.1.203", tapif=3, affinity="4"
    )
    ...
    framework.start_process(
        pifus_writer_path,
        args=f"192.168.1.201 -p 11337 -l HIGH -o {file_prefix}_HIGH",
        affinity="5",
    )
    framework.start_process(
        pifus_writer_path,
        args=f"192.168.1.202 -p 11337 -l MEDIUM -o {file_prefix}_MEDIUM",
        affinity="6",
    )
    framework.start_process(
        pifus_writer_path,
        args=f"192.168.1.203 -p 11337 -l LOW -o {file_prefix}_LOW",
        affinity="7",
    )
